...
- **`dtos/legacy-api.dto.ts`**: nuevos `ContratoEstadoDto`, `EntidadAseguradoraDto`, `PolizaDto`, `ApiResponseDto` (respuesta del PATCH); `Supervisor` con `Documento`; `NovedadMidDto` con índice laxo (los `GetNovedad*` no están documentados campo a campo).
